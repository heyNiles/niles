...
module.exports = {
  ...
  new: async (ctx, skillName) => {
    const { logger, homePath, shell } = ctx;

    if (!skillName) {
      throw new Error('Skill name required');
    }

    if (!fs.existsSync(path.join(homePath, 'skills'))) {
      fs.mkdirSync(path.join(homePath, 'skills'));
    }

    const skillPath = path.join(homePath, 'skills', skillName);
    ...
    shell.cd(skillPath);
    await shell.execAsync('git clone https://github.com/heyNiles/skill-template.git .');
    shell.rm('-rf', path.join(skillPath, '.git'));
    shell.sed('-i', '__SKILL_NAME__', skillName, path.join(skillPath, 'package.json'));

    logger.log(`Created at ${skillPath}`);
  },
